...
chrome.runtime.onMessage.addListener(async (params, sender, sendResponse) => {
    ...
    if (params.message === 'getRate') {
    ...
    } else if (params.message == 'down_taobao_detail') {
        let list = JSON.parse(params.data)
        console.log(list)
        for (let index = 0; index < list.length; index++) {
            ...
            chrome.downloads.download({
                url: item,
                saveAs: false,
                filename: 'taobao/' + index + '.jpg',
                conflictAction: 'uniquify'
            })
        }   
    } else if (params.message == 'downloadStock') {
        // 拿到值
        console.log('123123123', params.list)
        // 创一个二维数组
        let outList = [['SKU', '产品中文名称', '产品的总库存', '维赢美东', '维赢美西', '派派纽约', '派派迈阿密', '派派达拉斯', '派派洛杉矶', '派派新泽西']]
        // 循环
        let keys = [
            "MT-S1-Silver S1",
            "MT-ELE156in1-black",
        ...
        keys.forEach(key => {
            // 拿到对象的值
            let val = params.list[key]
            let current_arr = []
            current_arr.push(key, val.goodName, val.goodTotalNum, 0,0,0,0,0,0,0)
            // 这个地方要判断
            val.warehouseList.forEach(warehouse => {
                let position = map_obj[warehouse.warehouseName]
                if (position) {
                    // 对应位置的数字改掉
                    current_arr[position] = warehouse.omsAvailableQuantity
                }
            })
            // 导入
            outList.push(current_arr)
        })
        // 导出
        let wb = XLSX.utils.book_new()
        let ws = XLSX.utils.aoa_to_sheet(outList)
        // 合成
        let date = new Date()
        // 月
...
chrome.runtime.onMessageExternal.addListener(async (params, sender, sendResponse) => {
    ...
    if (params.message === 'demo') {
        ...
        for (let index = 0; index < list.length; index++) {
            ...
            result_obj = await countWordFrequency(element.page_alt, result_obj)
        }
